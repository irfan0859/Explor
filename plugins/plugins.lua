do
-- #Begin plugins.lua by @MeGaPlusTeaM
...
local function list_all_plugins(only_enabled, msg)
  ...
  text = '<code>'..text..'</code>\n\n'..nsum..' <b>⚡️plugins installed</b>\n\n'..nact..' <i>️️🌝plugins enabled</i>\n\n'..nsum-nact..' <i>🌚plugins disabled</i>'..tmp
...
local function list_plugins(only_enabled, msg)
  ...
  text = "`ربات اکسپلور با موفقیت بازنگری شد`"..msg_caption
...
local Clang = redis:get(Chash)
  ...
  if is_sudo(msg) then
  if (matches[1]:lower() == 'plist' and not Clang) or (matches[1]:lower() == 'پلاگین ها' and Clang) then --after changed to moderator mode, set only sudo
  ...
  if (matches[1]:lower() == 'reload' and not Clang) or (matches[1]:lower() == 'بازنگری' and Clang) and is_sudo(msg) then --after changed to moderator mode, set only sudo
    return reload_plugins(true, msg)
  end
end
