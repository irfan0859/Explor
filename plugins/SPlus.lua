--Begin Tools.lua :)
local SUDO = 559152990 -- put Your ID here! <===
...
local function warning(msg)
	...
	local d = math.floor(expiretime / 86400) + 1
        if tonumber(d) == 1 and not is_sudo(msg) and is_mod(msg) then
			if lang then
				tdbot.sendMessage(msg.to.id, 0, 1, '`یک روز گروه شما شارژ داری و پس اتمام خارج میشود برای شارژ مجدد به ادمین یا سودو مراجعه کنید`', 1, 'md')
			else
  ...
  if matches[2] and not string.match(matches[2], '^%d+$') then
   tdbot_function ({
      ...
      username = matches[2]
		...
		if ((matches[1] == "config" and not Clang) or (matches[1] == "ارتقا گروه" and Clang)) and is_admin(msg) then
...
if is_sudo(msg) then
   		if ((matches[1]:lower() == 'install' and not Clang) or (matches[1] == "نصب" and Clang)) and not redis:get('ExpireDate:'..msg.to.id) then
			...
			redis:setex('ExpireDate:'..msg.to.id, 180, true)
				...
				if lang then
					tdbot.sendMessage(msg.to.id, msg.id, 1, '', 1, 'md')
				else
					tdbot.sendMessage(msg.to.id, msg.id, 1, '', 1, 'md')
				end
		end
...
if ((matches[1] == 'setbotname' and not Clang) or (matches[1] == "تغییر نام" and Clang)) and is_sudo(msg) then
...
if ((matches[1] == 'markread' and not Clang) or (matches[1] == "تیک دوم" and Clang)) and is_sudo(msg) then
...
return chat_list(msg)
   ...
   if ((matches[1]:lower() == 'join' and not Clang) or (matches[1] == "افزودن" and Clang)) and is_admin(msg) and matches[2] then
	   ...
	   tdbot.sendMessage(matches[2], 0, 1, "MeGa Plus Admin Joined To Group", 1, 'html')
...
if ((matches[1] == 'megaplus' and not Clang) or (matches[1] == "مگا پلاس" and Clang)) then
