--Begin Utils.lua By #MeGaPlusTeaM :)
...
assert (tdbot_function ({
  ...
  if not lang then
    return tdbot.sendText(msg.chat_id, msg.id, "`All group admins has been promoted to MeGaPlus Bot`", 0, 1, nil, 0, 'md', 0, nil)
else
    return tdbot.sendText(msg.chat_id, msg.id, "`تمامی ادمین های گروه در ربات مگا پلاس ارتقا یافتند`", 0, 1, nil, 0, 'md', 0, nil)
     end
 end
...
local lang = redis:get(hash)
  ...
  if next(data[tostring(chat_id)]['banned']) == nil then --fix way
   ...
   message = '`لیست کاربران محروم شده از گروه :`\n'
     end
...
local lang = redis:get(hash)
  ...
  if next(data[tostring(chat_id)]['is_silent_users']) == nil then --fix way
   ...
   message = '`لیست کاربران سایلنت شده :`\n'
    end
...
local lang = redis:get(hash)
  ...
  if next(data['gban_users']) == nil then --fix way
   ...
   message = '`لیست کاربران محروم شده از گروه های ربات :`\n'
   end
